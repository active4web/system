...
redirect(DIR."site/".$controller_curt."/".$curt_sub);    
    ...
    function index() {
        ...
        $tables = "team_work";
        $config['base_url'] = base_url().'site/doctors/team_work'; 
	...
	function doctor_details() {
		...
		$page_data['product'] =$this->db->get_where('team_work',array('id'=>$id_product))->result(); 
		$data['lang'] =$lang; 
		$data_contant['lang'] =$lang; 
		$viewed_num = $this->data->get_table_row('team_work',array('id'=>$id_product),'viewed_num');
		$main_product_data['viewed_num']=$viewed_num+1;
		$this->db->update('team_work',$main_product_data,array('id'=>$id_product));
	$this->load->view('include/head',$data );
	$this->load->view('include/insideheader',$data );
	$this->load->view('doctor_details',$page_data);
