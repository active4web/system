...
redirect(DIR."site/".$controller_curt."/".$curt_sub);    
    ...
    function index() {
        ...
        $tables = "product";
        $config['base_url'] = base_url().'site/product/products'; 
	...
	function products_details() {
		...
		$page_data['product'] =$this->db->get_where('product',array('id'=>$id_product))->result(); 
		$page_data['gallery'] =$this->db->get_where('gallery',array('id_tab'=>$id_product,'type'=>'0'))->result(); 
		$page_data['gallery_count'] =$this->db->get_where('gallery',array('id_tab'=>$id_product,'type'=>'0'))->result(); 
		$data['lang'] =$lang; 
		$data_contant['lang'] =$lang; 
		$viewed_num = $this->data->get_table_row('product',array('id'=>$id_product),'viewed_num');
		$main_product_data['viewed_num']=$viewed_num+1;
		$this->db->update('product',$main_product_data,array('id'=>$id_product));

	$this->load->view('include/head',$data );
	$this->load->view('include/insideheader',$data );
	$this->load->view('products_details',$page_data);
