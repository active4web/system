...
redirect(DIR."site/".$controller_curt."/".$curt_sub);    
    ...
    function index() {
        ...
        $config['base_url'] = base_url().'site/stories/success_stories'; 
       ...
       // $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
		...
		$datahead['lang'] =$lang; 
		$data['lang'] =$lang;
		$data['result_amount'] =$this->db->get_where('success_stories',array('view'=>'1'))->result(); 
        else:
		...
		$this->load->view('success_stories',$data);
	...
	function story_details() {
		...
		$page_data['product'] =$this->db->get_where('success_stories',array('id'=>$id_product))->result(); 
		$page_data['gallery'] =$this->db->get_where('gallery',array('id_tab'=>$id_product,'type'=>'1'))->result(); 
		$page_data['gallery_count'] =$this->db->get_where('gallery',array('id_tab'=>$id_product,'type'=>'1'))->result(); 
		$data['lang'] =$lang; 
		$data_contant['lang'] =$lang; 
		$viewed_num = $this->data->get_table_row('success_stories',array('id'=>$id_product),'viewed_num');
		$main_product_data['viewed_num']=$viewed_num+1;
		$this->db->update('success_stories',$main_product_data,array('id'=>$id_product));

	$this->load->view('include/head',$data );
	$this->load->view('include/insideheader',$data );
	$this->load->view('story_details',$page_data);
